...
import { Icon, MarkText, Paper, PlaceholderImg } from "@/lib/design/shared";
	...
	return (
		<Paper style={s.page}>
			...
			<Link
				href="/artworks"
				style={s.back}
				className="px-5 pt-6 md:px-16 md:pt-7"
			>
			...
			<div
				style={s.titleBlock}
				className="grid-cols-1 gap-6 px-5 py-8 md:grid-cols-[1.4fr_1fr] md:gap-14 md:px-16 md:pt-10 md:pb-12"
			>
				<div>
					<div style={s.plateNum}>
						{artwork.medium.split(" · ")[0].toUpperCase()}
					</div>
				...
				<div style={s.meta}>
					...
					<div style={s.metaRow}>
						...
						<span>
							{artwork.artworkTags.map((at) => at.tag.name).join(", ")}
						</span>
					</div>
					<div style={s.metaRow}>
			...
			<div
				style={s.footer}
				className="grid-cols-1 gap-10 px-5 py-12 md:grid-cols-[1.3fr_1fr] md:gap-14 md:px-16 md:py-24"
			>
				<div>
				...
				<div>
					...
					<div style={s.contact}>
						...
						<a
							href={`mailto:${CONTACT_EMAIL}`}
							style={{ ...s.contactBtn, background: COLORS.accent }}
						>
							Let&apos;s talk!{" "}
							<Icon name="arrow" size={14} color={COLORS.cream} />
						</a>
					</div>
				</div>
			</div>
